...
export function StudentDashboardPage() {
  ...
  const activeAppsCount = appsList.filter(
  ...
  // "active": true
  //
  // Older frontend type may use:
  // "isActive": true
  //
  // We support both here so the dashboard works with either shape.
  // ------------------------------------------------------------
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <Card className="hover:border-indigo-200 transition-all">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {skills.length} verified skills in profile
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-indigo-200 transition-all">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {appsList.length} total applications submitted
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-indigo-200 transition-all">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {activeResume
                ? `v${activeResume.version} • Active for applications`
                : 'Upload a PDF to apply'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex items-center justify-between">
          ...
          >
            View All ({applications?.totalElements || 0})
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500">
                      {app.companyName} &bull; Applied on{' '}
                  ...
                  <div className="flex items-center gap-4">
                    <ScoreGauge
                      score={app.matchScoreAtApplication}
                      size="sm"
                    />
